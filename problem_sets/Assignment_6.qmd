...
-   This homework is due by **10pm on Thursday 10/09/2025**. Please upload it using your personal GitHub repository for this class.

-   Please name your Quarto file `assignment_6.qmd` and the rendered markdown file `assignment_6.md`.

-   For this assignment, please **reproduce this markdown file** using Quarto. This includes the followings:
    ...
    -   Have all your code embedded within the Quarto file, and show **BOTH your code and plots** in the knitted markdown file.
...
-   Please note that you only need to finish **three out of the five** questions in **Exercise 1** in order to get credit for that exercise. Also, **Exercise 3** is **optional**.
...
## 2.a error message abt values/characters 

weather_raw2 <- weather_raw |> 
  mutate(across(3:ncol(weather_raw), as.character())
         
         view(weather_raw2)
## Part 2.b: Tidy format 
...
weather_long3 <- weather_long2 |> 
  mutate(
    time = hm(paste0(hour, ":00")),  # combine hour → time
    value = as.numeric(ifelse(value == "NR", 0, value))  # handle NR
